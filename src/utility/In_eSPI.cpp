...
spi_host_device_t spi_host =
    (spi_host_device_t)DMA_CHANNEL;  // Draws once then freezes
#else  // use FSPI port
#define DMA_CHANNEL 1
spi_host_device_t spi_host =
    (spi_host_device_t)DMA_CHANNEL;  // Draws once then freezes
...
/***************************************************************************************
** Function name:           GPIO direction control  - supports class functions
** Description:             Set ESP32 GPIO pin to input or output (set high)
*ASAP
***************************************************************************************/
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    !defined(TFT_PARALLEL_8_BIT)  // Code for RPi TFT
////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    !defined(TFT_PARALLEL_8_BIT)  // Most SPI displays
////////////////////////////////////////////////////////////////////////////////////////
...
void TFT_eSPI::pushBlock(uint16_t color, uint32_t len){
  ...
  while ((*_spi_cmd)&SPI_USR); // Move to later in code to use transmit time
usefully?
}
//*/
//*
void TFT_eSPI::pushBlock(uint16_t color, uint32_t len) {
    ...
    uint32_t color32 =
        (color << 8 | color >> 8) << 16 | (color << 8 | color >> 8);
    uint32_t i   = 0;
...
void TFT_eSPI::pushSwapBytePixels(const void *data_in, uint32_t len) {
    ...
    if (len) {
        ...
        for (uint32_t i = 0; i <= (len << 1); i += 4) {
            WRITE_PERI_REG(SPI_W0_REG(SPI_PORT) + i, DAT8TO32(data));
            data += 4;
        }
#if CONFIG_IDF_TARGET_ESP32S3
        SET_PERI_REG_MASK(SPI_CMD_REG(SPI_PORT), SPI_UPDATE);
...
/***************************************************************************************
** Function name:           pushSwapBytePixels - for ESP32 and 3 byte RGB
*display
** Description:             Write a sequence of pixels with swapped bytes
***************************************************************************************/
void TFT_eSPI::pushSwapBytePixels(const void *data_in, uint32_t len) {
    uint16_t *data = (uint16_t *)data_in;
    // ILI9488 write macro is not endianess dependant, so swap byte macro not
    // used here
    while (len--) {
        tft_Write_16(*data);
        data++;
...
***************************************************************************************/
// This will byte swap the original image if setSwapBytes(true) was called by
// sketch.
...
void TFT_eSPI::pushImageDMA(int32_t x, int32_t y, int32_t w, int32_t h,
    ...
    if ((dw != w) || (dh != h)) {
        if (_swapBytes) {
            ...
            for (int32_t yb = 0; yb < dh; yb++) {
                memcpy((uint8_t *)(buffer + yb * dw),
                       (uint8_t *)(image + dx + w * (yb + dy)), dw << 1);
            }
        }
    }
    // else, if a buffer pointer has been provided copy whole image to the
    // buffer
    else if (buffer != image || _swapBytes) {
        if (_swapBytes) {
            for (uint32_t i = 0; i < len; i++)
                (buffer[i] = image[i] << 8 | image[i] >> 8);
        } else {
...
#endif
        CS_L;
        SET_BUS_WRITE_MODE;  // Some processors (e.g. ESP32) allow recycling the
                             // tx buffer when rx is not used
    }
}
...
inline void TFT_eSPI::end_tft_write(void) {
    if (!inTransaction) {    // Flag to stop ending transaction during multiple
                             // graphics calls
...
inline void TFT_eSPI::begin_tft_read(void) {
    DMA_BUSY_CHECK;  // Wait for any DMA transfer to complete before changing
                     // SPI settings
#if defined(SPI_HAS_TRANSACTION) && defined(SUPPORT_TRANSACTIONS) && \
    !defined(TFT_PARALLEL_8_BIT) && !defined(RP2040_PIO_INTERFACE)
    if (locked) {
...
void TFT_eSPI::setViewport(int32_t x, int32_t y, int32_t w, int32_t h,
    ...
    _vpDatum = false;  // Datum is at top left corner of screen (true = top left
                       // of viewport)
    _vpOoB =
        false;  // Out of Bounds flag (true is all of viewport is off screen)
    _vpX = 0;   // Viewport top left corner x coordinate
    _vpY = 0;   // Viewport top left corner y coordinate
    _vpW = width();   // Equivalent of TFT width  (Nb: viewport right edge coord
                      // + 1)
    _vpH = height();  // Equivalent of TFT height (Nb: viewport bottom edge
                      // coord + 1)
    ...
    // y); Serial.print(", w=");Serial.print(w);Serial.print(",
    // h=");Serial.println(h);
    ...
    if (!vpDatum) {
        _xDatum =
            0;  // Reset to top left of screen if not using a viewport datum
        _yDatum  = 0;
        _xWidth  = width();
        _yHeight = height();
    }
    ...
    // Serial.print(" _xDatum=");Serial.print( _xDatum);Serial.print(",
    // _yDatum=");Serial.print( _yDatum); Serial.print(",
    // _xWidth=");Serial.print(_xWidth);Serial.print(",
    // _yHeight=");Serial.println(_yHeight);

    // Serial.print(" _vpX=");Serial.print( _vpX);Serial.print(",
    // _vpY=");Serial.print( _vpY); Serial.print(",
    // _vpW=");Serial.print(_vpW);Serial.print(", _vpH=");Serial.println(_vpH);
}

/***************************************************************************************
** Function name:           checkViewport
** Description:             Check if any part of specified area is visible in
*viewport
***************************************************************************************/
...
** Description:             Draw a frame inside or outside the viewport of width
*w
***************************************************************************************/
void TFT_eSPI::frameViewport(uint16_t color, int32_t w) {
    ...
    // a large negative width will clear the screen outside the viewport
    {
        ...
        // Draw frame
        fillRect(_xT - w - _xDatum, _yT - w - _yDatum, _wT - _xT + w + w, w,
                 color);  // Top
        fillRect(_xT - w - _xDatum, _yT - _yDatum, w, _hT - _yT,
                 color);                                              // Left
        fillRect(_wT - _xDatum, _yT - _yDatum, w, _hT - _yT, color);  // Right
        fillRect(_xT - w - _xDatum, _hT - _yDatum, _wT - _xT + w + w, w,
                 color);  // Bottom
...
TFT_eSPI::TFT_eSPI(int16_t w, int16_t h) {
    ...
    lockTransaction = false;  // start/endWrite lock flag to allow sketch to
                              // keep SPI bus access open
    ...
    fs_font =
        true;  // Smooth font filing system or array (fs_font = false) flag
#endif
...
// In parallel mode and with the RP2040 processor, the TFT_WR line is handled in
// the  PIO
#if defined(TFT_WR) && !defined(ARDUINO_ARCH_RP2040) && \
    ...
    !defined(ARDUINO_ARCH_MBED)  // PIO manages pins
...
// Legacy bitmasks for GPIO
    ...
    !defined(ARDUINO_ARCH_RP2040) && !defined(ARDUINO_ARCH_MBED)
        ...
        pinMode(TFT_CS, OUTPUT);
        digitalWrite(TFT_CS, HIGH);  // Chip select high (inactive)
#elif defined(ARDUINO_ARCH_ESP8266) && !defined(TFT_PARALLEL_8_BIT) && \
    !defined(RP2040_PIO_SPI)
    ...
    if (TFT_RST >= 0) {
        writecommand(
            0x00);  // Put SPI bus in known state for TFT with CS tied low
...
#include "In_eSPI_Drivers/ILI9341_Init.h"

#elif defined(ST7735_DRIVER)
    tabcolor = tc;
#include "In_eSPI_Drivers/ST7735_Init.h"
...
#include "In_eSPI_Drivers/ST7796_Init.h"
...
#include "In_eSPI_Drivers/SSD1351_Init.h"

#elif defined(SSD1963_DRIVER)
#include "In_eSPI_Drivers/SSD1963_Init.h"

#elif defined(GC9A01_DRIVER)
#include "In_eSPI_Drivers/GC9A01_Init.h"

#elif defined(ILI9225_DRIVER)
#include "In_eSPI_Drivers/ILI9225_Init.h"

#elif defined(RM68120_DRIVER)
#include "In_eSPI_Drivers/RM68120_Init.h"

#elif defined(HX8357B_DRIVER)
#include "In_eSPI_Drivers/HX8357B_Init.h"

#elif defined(HX8357C_DRIVER)
#include "In_eSPI_Drivers/HX8357C_Init.h"
...
#if defined(ILI9341_DRIVER) || defined(ILI9341_2_DRIVER) || \
    defined(ILI9342_DRIVER)
#include "In_eSPI_Drivers/ILI9341_Rotation.h"

#elif defined(ST7735_DRIVER)
#include "In_eSPI_Drivers/ST7735_Rotation.h"
...
#elif defined(ST7796_DRIVER)
#include "In_eSPI_Drivers/ST7796_Rotation.h"
...
#elif defined(SSD1351_DRIVER)
#include "In_eSPI_Drivers/SSD1351_Rotation.h"

#elif defined(SSD1963_DRIVER)
#include "In_eSPI_Drivers/SSD1963_Rotation.h"

#elif defined(GC9A01_DRIVER)
#include "In_eSPI_Drivers/GC9A01_Rotation.h"

#elif defined(ILI9225_DRIVER)
#include "In_eSPI_Drivers/ILI9225_Rotation.h"

#elif defined(RM68120_DRIVER)
#include "In_eSPI_Drivers/RM68120_Rotation.h"

#elif defined(HX8357B_DRIVER)
#include "In_eSPI_Drivers/HX8357B_Rotation.h"

#elif defined(HX8357C_DRIVER)
#include "In_eSPI_Drivers/HX8357C_Rotation.h"
...
uint16_t TFT_eSPI::readPixel(int32_t x0, int32_t y0) {
...
#if defined(ILI9341_DRIVER) || defined(ILI9341_2_DRIVER) || \
    defined(ILI9488_DRIVER) || defined(SSD1963_DRIVER)  // Read 3 bytes
    ...
    /*
      ...
      (tft_Read_8()&0x7E)<<1; uint8_t b = (tft_Read_8()&0x7E)<<1; color =
      color565(r, g, b);
...
void TFT_eSPI::readRect(int32_t x, int32_t y, int32_t w, int32_t h,
...
#if defined(ILI9341_DRIVER) || defined(ILI9341_2_DRIVER) || \
    defined(ILI9488_DRIVER)  // Read 3 bytes
    // Dummy read to throw away don't care value
    readByte();
    ...
    while (dh--) {
        ...
        while (lw--) {
            ...
            uint16_t rgb = ((readByte() & 0xF8) << 8) |
                           ((readByte() & 0xFC) << 3) | (readByte() >> 3);
    ...
    while (dh--) {
        ...
        while (lw--) {
            ...
            color = ((tft_Read_8()) << 8) | (tft_Read_8());
#else
            // Read the 3 RGB bytes, colour is actually only in the top 6 bits
            // of each byte as the TFT stores colours as 18 bits
            uint8_t r = tft_Read_8();
            uint8_t g = tft_Read_8();
            uint8_t b = tft_Read_8();
            color     = color565(r, g, b);
            ...
            // The 6 colour bits are in MS 6 bits of each byte but we do not
            // include the extra clock pulse so we use a trick and mask the
            // middle 6 bits of the byte, then only shift 1 place left
...
void TFT_eSPI::pushImage(int32_t x, int32_t y, int32_t w, int32_t h,
    ...
    // The little endian transp color must be byte swapped if the image is big
    // endian
...
void TFT_eSPI::pushImage(int32_t x, int32_t y, int32_t w, int32_t h,
    ...
    } else if (cmap != nullptr)  // Must be 4bpp
    {
        ...
        w = (w + 1) & 0xFFFE;  // if this is a sprite, w will already be even;
                               // this does no harm.
        bool splitFirst =
            (dx & 0x01) != 0;  // split first means we have to push a single px
                               // from the left of the sprite / image
    ...
    } else                  // Must be 1bpp
    {
        ...
        for (int32_t yp = dy; yp < dy + dh; yp++) {
            ...
            for (int32_t xp = dx; xp < dx + dw; xp++) {
                uint16_t col =
                    (pgm_read_byte(ptr + (xp >> 3)) & (0x80 >> (xp & 0x7)));
                if (col) {
                    *linePtr++ = bitmap_fg >> 8;
    ...
    } else if (cmap != nullptr)  // 4bpp with color map
    {
        ...
        while (dh--) {
            ...
            while (len--) {
                ...
                // find the actual color you care about.  There will be two
                // pixels here! but we may only want one at the end of the row
                uint16_t index = ((color & 0xF0) >> 4) &
                                 0x0F;  // high bits are the even numbers
                if (index != transp) {
                    if (move) {
                        move = false;
                        sx   = px;
...
// sprites) Improved algorithm avoids repetition of lines
...
// background is not filled. If bgfill = true, then a smooth font background
// fill will be used.
...
void TFT_eSPI::drawChar(int32_t x, int32_t y, uint16_t c, uint32_t color,
    ...
    if (!gfxFont) {  // 'Classic' built-in font
        ...
        if ((size == 1) && fillbg && !clip) {
            ...
            // begin_tft_write();          // Sprite class can use this
            // function, avoiding begin_tft_write()
            inTransaction = true;
...
** Description:             define an area to receive a stream of pixels
***************************************************************************************/
// Chip select stays low, call begin_tft_write first. Use setAddrWindow() from
// sketches
...
// Temporary solution is to include the RP2040 code here
    ...
    // For ILI9225 and RP2040 the slower Arduino SPI transfer calls were used,
    // so need to swap back to 16 bit mode
...
// DC set high by PIO
#if defined(SPI_18BIT_DRIVER)
    TX_FIFO = ((color & 0xF800) << 8) | ((color & 0x07E0) << 5) |
              ((color & 0x001F) << 3);
#else
    TX_FIFO = color;
#endif
...
void TFT_eSPI::drawLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
    ...
    // x+= _xDatum;             // Not added here, added by drawPixel &
    // drawFastXLine y+= _yDatum;
...
***************************************************************************************/
void TFT_eSPI::fillSmoothCircle(int32_t x, int32_t y, int32_t r, uint32_t color,
                                uint32_t bg_color) {
    if (r <= 0) return;
...
/***************************************************************************************
** Function name:           drawSpot - maths intensive, so for small filled
*circles
** Description:             Draw an anti-aliased filled circle at ax,ay with
*radius r
***************************************************************************************/
...
/***************************************************************************************
** Function name:           drawWideLine - background colour specified or pixel
*read
...
/***************************************************************************************
** Function name:           drawWedgeLine - background colour specified or pixel
*read
...
void TFT_eSPI::drawWedgeLine(float ax, float ay, float bx, float by, float ar,
    ...
    for (int32_t yp = ys; yp <= y1; yp++) {
        ...
        for (int32_t xp = xs; xp <= x1; xp++) {
            ...
            pushColor(
                alphaBlend((uint8_t)(alpha * PixelAlphaGain), fg_color, bg));
        }
    }
...
/***************************************************************************************
** Function name:           lineDistance - private helper function for
*drawWedgeLine
...
void TFT_eSPI::setAttribute(uint8_t attr_id, uint8_t param) {
    switch (attr_id) {
        ...
        case PSRAM_ENABLE:
            ...
            if (psramFound())
                _psram_enable =
                    param;  // Enable the use of PSRAM (if available)
            else
#endif
                _psram_enable = false;
            break;
...
uint16_t TFT_eSPI::alphaBlend(uint8_t alpha, uint16_t fgc, uint16_t bgc) {
    // For speed use fixed point maths and rounding to permit a power of 2
    // division
    ...
    // return ((r&0x18) << 11) | ((g&0x30) << 5) | ((b&0x18) << 0); // 2 bit
    // greyscale return ((r&0x1E) << 11) | ((g&0x3C) << 5) | ((b&0x1E) << 0); //
    // 4 bit greyscale
    return (r << 11) | (g << 5) | (b << 0);
}
...
uint16_t TFT_eSPI::alphaBlend(uint8_t alpha, uint16_t fgc, uint16_t bgc,
    ...
    if (dither) {
        int16_t alphaDither = (int16_t)alpha - dither +
                              random(2 * dither + 1);  // +/-4 randomised
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if (!gfxFont) {
        ...
        if (textfont == 2) {
            ...
            cwidth =
                (cwidth + 6) / 8;  // Width in whole bytes for font 2, should be
...
int16_t TFT_eSPI::drawChar(uint16_t uniCode, int32_t x, int32_t y,
    ...
    if (font == 2) {
        ...
        if (textcolor == textbgcolor || textsize != 1 || clip) {
            ...
            inTransaction = lockTransaction;
            end_tft_write();
        } else {  // Faster drawing of characters and background using block
                  // write
            ...
            for (int32_t i = 0; i < height; i++) {
                ...
                for (int32_t k = 0; k < w; k++) {
                    line =
                        pgm_read_byte((uint8_t *)(flash_address + w * i + k));
                    mask = 0x80;
                    while (mask && pX) {
                        if (line & mask) {
    ...
    // Font is not 2 and hence is RLE encoded
    {
        ...
        if (textcolor == textbgcolor && !clip) {
            ...
            // Text colour != background and textsize = 1 and character is
            // within viewport area so use faster drawing of characters and
            // background using block write
            if (textcolor != textbgcolor && textsize == 1 && !clip) {
                setWindow(xd, yd, xd + width - 1, yd + height - 1);
...
int16_t TFT_eSPI::drawString(const char *string, int32_t poX, int32_t poY,
    ...
    // vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv DEBUG
    // vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
    //  This is debug code to show text (green box) and blanked (white box)
    //  areas It shows that the padding areas are being correctly sized and
    //  positioned
...
** Description:             Compare the ID if USER_SETUP_ID defined in user
*setup file
***************************************************************************************/
