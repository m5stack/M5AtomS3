...
#define IR_TOOLS_FLAGS_INVERSE                                                 \
    (1 << 1) /*!< Inverse the IR signal, i.e. take high level as low, and vice \
                versa */
...
struct ir_builder_s {
     ...
     *      - ESP_FAIL: Build a complete frame failed because some error
     * occurred
     */
    esp_err_t (*build_frame)(ir_builder_t *builder, uint32_t address,
                             uint32_t command);
     ...
     *
     * @param[in] builder: Handle of IR builder
     * @param[out] result: Result of frame building, which contains all of the
     * raw data that could be send directly
     * @param[out] length: Length of result data
     *
     * @return
     *      - ESP_OK: Get result data successfully
     *      - ESP_ERR_INVALID_ARG: Get result data failed because of invalid
     * arguments
     *      - ESP_FAIL: Get result data failed because some other errors
     * occurred
     */
    esp_err_t (*get_result)(ir_builder_t *builder, void *result,
                            size_t *length);
...
struct ir_parser_s {
     ...
     *      - ESP_ERR_INVALID_ARG: Input raw data failed because of invalid
     * argument
     ...
     *      - ESP_ERR_INVALID_ARG: Get scan code failed because of invalid
     * arguments
     *      - ESP_FAIL: Get scan code failed because some error occurred
     */
    esp_err_t (*get_scan_code)(ir_parser_t *parser, uint32_t *address,
                               uint32_t *command, bool *repeat);
...
typedef struct {
    ...
    uint32_t flags;       /*!< Flags for IR builder, different flags will enable
                             different features */
} ir_builder_config_t;
...
typedef struct {
    ir_dev_t dev_hdl;   /*!< IR device handle */
    uint32_t flags;     /*!< Flags for IR parser, different flags will enable
                           different features */
    uint32_t margin_us; /*!< Timing parameter, indicating the tolerance to
                           environment noise */
} ir_parser_config_t;
...
#define IR_BUILDER_DEFAULT_CONFIG(dev) \
    { .buffer_size = 64, .dev_hdl = dev, .flags = 0, }
...
#define IR_PARSER_DEFAULT_CONFIG(dev) \
    { .dev_hdl = dev, .flags = 0, .margin_us = 200, }
