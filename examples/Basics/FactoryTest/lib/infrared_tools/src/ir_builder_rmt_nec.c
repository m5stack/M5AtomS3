...
static esp_err_t nec_builder_make_head(ir_builder_t *builder) {
    ...
    nec_builder->buffer[nec_builder->cursor].duration0 =
        nec_builder->leading_code_high_ticks;
    nec_builder->buffer[nec_builder->cursor].level1 = nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration1 =
        nec_builder->leading_code_low_ticks;
...
static esp_err_t nec_builder_make_logic0(ir_builder_t *builder) {
    ...
    nec_builder->buffer[nec_builder->cursor].level0 = !nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration0 =
        nec_builder->payload_logic0_high_ticks;
    nec_builder->buffer[nec_builder->cursor].level1 = nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration1 =
        nec_builder->payload_logic0_low_ticks;
    nec_builder->cursor += 1;
    return ESP_OK;
}
...
static esp_err_t nec_builder_make_logic1(ir_builder_t *builder) {
    ...
    nec_builder->buffer[nec_builder->cursor].level0 = !nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration0 =
        nec_builder->payload_logic1_high_ticks;
    nec_builder->buffer[nec_builder->cursor].level1 = nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration1 =
        nec_builder->payload_logic1_low_ticks;
    nec_builder->cursor += 1;
    return ESP_OK;
}
...
static esp_err_t nec_builder_make_end(ir_builder_t *builder) {
    ...
    nec_builder->buffer[nec_builder->cursor].level0 = !nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration0 =
        nec_builder->ending_code_high_ticks;
    nec_builder->buffer[nec_builder->cursor].level1 = nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration1 =
        nec_builder->ending_code_low_ticks;
    nec_builder->cursor += 1;
...
static esp_err_t nec_build_frame(ir_builder_t *builder, uint32_t address,
                                 uint32_t command) {
    ...
    if (!nec_builder->flags & IR_TOOLS_FLAGS_PROTO_EXT) {
        ...
        NEC_CHECK(low_byte == (~high_byte & 0xFF),
                  "address not match standard NEC protocol", err,
                  ESP_ERR_INVALID_ARG);
        low_byte  = command & 0xFF;
        high_byte = (command >> 8) & 0xFF;
        NEC_CHECK(low_byte == (~high_byte & 0xFF),
                  "command not match standard NEC protocol", err,
                  ESP_ERR_INVALID_ARG);
    }
...
static esp_err_t nec_build_repeat_frame(ir_builder_t *builder) {
    ...
    nec_builder->buffer[nec_builder->cursor].level0 = !nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration0 =
        nec_builder->repeat_code_high_ticks;
    nec_builder->buffer[nec_builder->cursor].level1 = nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration1 =
        nec_builder->repeat_code_low_ticks;
    nec_builder->cursor += 1;
    nec_builder_make_end(builder);
    return ESP_OK;
}

static esp_err_t nec_builder_get_result(ir_builder_t *builder, void *result,
                                        size_t *length) {
    esp_err_t ret              = ESP_OK;
    nec_builder_t *nec_builder = __containerof(builder, nec_builder_t, parent);
    NEC_CHECK(result && length, "result and length can't be null", err,
              ESP_ERR_INVALID_ARG);
...
ir_builder_t *ir_builder_rmt_new_nec(const ir_builder_config_t *config) {
    ...
    uint32_t builder_size =
        sizeof(nec_builder_t) + config->buffer_size * sizeof(rmt_item32_t);
    ...
    nec_builder->ending_code_high_ticks =
        (uint32_t)(ratio * NEC_ENDING_CODE_HIGH_US);
