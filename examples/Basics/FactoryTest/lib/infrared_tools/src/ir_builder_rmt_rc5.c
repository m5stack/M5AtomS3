...
static esp_err_t rc5_builder_make_head(ir_builder_t *builder) {
    ...
    rc5_builder->buffer[rc5_builder->cursor].duration0 =
        rc5_builder->pulse_duration_ticks;
    rc5_builder->buffer[rc5_builder->cursor].level1 = !rc5_builder->inverse;
    rc5_builder->buffer[rc5_builder->cursor].duration1 =
        rc5_builder->pulse_duration_ticks;
    rc5_builder->cursor += 1;
    // S2 default (not inverse) is depend on whether use extended protocol
    rc5_builder->buffer[rc5_builder->cursor].level0 =
        rc5_builder->s2_bit ^ rc5_builder->inverse;
    rc5_builder->buffer[rc5_builder->cursor].duration0 =
        rc5_builder->pulse_duration_ticks;
    rc5_builder->buffer[rc5_builder->cursor].level1 =
        !(rc5_builder->s2_bit ^ rc5_builder->inverse);
    rc5_builder->buffer[rc5_builder->cursor].duration1 =
        rc5_builder->pulse_duration_ticks;
    rc5_builder->cursor += 1;
...
static esp_err_t rc5_build_frame(ir_builder_t *builder, uint32_t address,
                                 uint32_t command) {
    rc5_builder_t *rc5_builder = __containerof(builder, rc5_builder_t, parent);
    if (rc5_builder->flags & IR_TOOLS_FLAGS_PROTO_EXT) {
        // RC5-extended protocol uses S2 bit as a 7th command bit (MSB of a
        // command)
        if (command > 63) {
...
static esp_err_t rc5_builder_get_result(ir_builder_t *builder, void *result,
                                        size_t *length) {
    esp_err_t ret              = ESP_OK;
    rc5_builder_t *rc5_builder = __containerof(builder, rc5_builder_t, parent);
    RC5_CHECK(result && length, "result and length can't be null", err,
              ESP_ERR_INVALID_ARG);
...
ir_builder_t *ir_builder_rmt_new_rc5(const ir_builder_config_t *config) {
    ...
    uint32_t builder_size =
        sizeof(rc5_builder_t) + config->buffer_size * sizeof(rmt_item32_t);
